...
class BurgerKing(object):
    ...
    def updateHistograms(self, imgBGR):
        ...
        # Определение цветового диапазона в HSV и создание маскиd
        lower_range = np.array((155., 50., 50.))
...
def iz_part2(file):
    ...
    writer = cv2.VideoWriter(r"D:\PythonProjects\DMPA\ind1\burger.mp4", cv2.VideoWriter_fourcc(*'mp4v'), 90, (int(cap.get(cv2.CAP_PROP_FRAME_WIDTH)), int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))))
    ...
    camShiftTracker = BurgerKing(bbox, frame)
...
files = ['D:/PythonProjects/DMPA/ind1/videos/dota4.mp4']
